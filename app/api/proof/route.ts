...
export async function POST(request: Request) {
  ...
  if (file instanceof File && file.size > 0) {
    ...
    const safe = file.name.replace(/[^a-zA-Z0-9._-]/g, "").slice(0, 80) || "proof";
    const dir = path.join(process.cwd(), "data", "uploads", member.team_id);
  ...
  try {
    addProof(userId, taskId, note, filePath, fileName);
